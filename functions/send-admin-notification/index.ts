// Portable version of the admin-notification edge function.
// Deploy this to YOUR Supabase project (Edge Functions → new function: send-admin-notification).
// Removes Lovable Cloud connector gateway — calls Resend REST API directly.
//
// Required secrets (Supabase → Project Settings → Edge Functions → Secrets):
//   RESEND_API_KEY   — from https://resend.com/api-keys
//   ADMIN_EMAIL      — recipient inbox
//   FROM_EMAIL       — verified sender, e.g. "TeknoKerja <no-reply@teknokerja.com>"
//
// config.toml (in your own repo):
...
const FROM = Deno.env.get("FROM_EMAIL") ?? "TeknoKerja <onboarding@resend.dev>";

type NotifType = "booking" | "contact" | "whatsapp_lead";
interface Payload { type: NotifType; data: Record<string, unknown>; }
  ...
  try {
    ...
    const html = `
      <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#fff">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:14px;border:1px solid #eee;border-radius:8px;overflow:hidden">${renderRows(data)}</table>
